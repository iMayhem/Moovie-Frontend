...
export async function registerUser(username: string, password: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        localStorage.setItem('moovie_current_user', cleanUsername);
        localStorage.setItem('watch_username', cleanUsername); // Seamless Watch Together sync!
        localStorage.setItem('watchlist', '[]');

        window.dispatchEvent(new Event('moovie_auth_change'));
        return { success: true };
    } catch (e) {
        console.error(e);
...
export function logoutUser() {
    ...
    localStorage.removeItem('moovie_current_user');
...
export function getCurrentUser(): string | null {
    ...
    return localStorage.getItem('moovie_current_user');
}
...
export async function syncUserDataWithSupabase(username: string) {
    ...
    try {
        ...
        if (!error && user) {
            ...
            window.dispatchEvent(new Event('moovie_userdata_change'));
        }
    } catch (e) {
